...
class GeneticAlgorithmAPS:
    ...
    def get_operation_list(self, individual: List[int]) -> List[Operation]:
        ...
        if not self.operation_list:
            ...
            for i in individual:
                ...
                self.operation_list += [op for op in wo.operations if op not in self.operation_list]
            self.operation_list.sort(key=lambda op: op.order_id*1000 + op.operation_id)  # 按照工单编号和操作编号排序
    ...
    def get_start_time(self, op: Operation) -> int:
        ...
        for resource, quantity in resources.items():
            ...
            processing_time = op.processing_time / efficiency
            start_time = max(start_time, max([t for t, q in usage[resource].items() if q >= quantity], default=start_time))
            for t in range(start_time, int(start_time + processing_time)):
    ...
    def fitness(self, individual: List[int]) -> float:
        ...
        delays = [max(0, self.end_times[i] - wo.due_date) for i, wo in enumerate(self.work_orders, start=1)]
